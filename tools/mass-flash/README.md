...
Flashes 32 ESP8266 boards with WLED 0.15 + Experts Live 5 Years config in one USB step per board. After each flash the script discovers the board's IP on the local network (via MAC in ARP) and prints a clickable URL.
...
**mDNS collisions:** all 32 boards claim the same mDNS name `expertslive`. Only one can hold the name at a time, so hostname-based discovery (`expertslive.local`) is unreliable in bulk. The script uses ARP + MAC match instead — reliable even with many identical devices on one LAN.
...
**IP not found within 60s:** the board flashed successfully but hasn't joined Wi-Fi yet. Wait another 30s and check your router's DHCP list, or re-run `./flash.sh --port <same-path>` to trigger another MAC lookup without re-flashing.
